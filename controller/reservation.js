...
exports.my_order_record = async (req, res, next) => {
  ...
  let query = `select group_concat(menu separator' ') as menu , sum(replace(replace(price, ","," "), "원", " ")) as price , created_at  from beauty_reservation where nick_name = "${nick_name}"`;
